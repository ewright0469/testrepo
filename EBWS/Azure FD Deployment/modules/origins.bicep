...
resource fdendpoint 'Microsoft.Cdn/profiles/afdEndpoints@2022-05-01-preview' existing = {
  name: 'placeholder'
  parent:fdprofile
}


resource originGroupDeploy 'Microsoft.Cdn/profiles/originGroups@2021-06-01' = [for origingroup in originGroups:{
...
resource routeCollection 'Microsoft.Cdn/profiles/afdEndpoints/routes@2021-06-01' = [for (route,i) in originGroups:{
  name: 'route-${originGroups[i].name}}'
  parent: fdendpoint
  dependsOn: [
    origin
    customDomains
  ...
  properties: {
    ...
    customDomains: [
      {
        id: contains(origins[i], backendPools[i].IP)? origins[i].domain: []
      }
    ]
    
    customDomains: [for (domain,i) in origins: {
      id: contains(domain, route.IP) ? customDomains[i].id: []
    }]
    */
